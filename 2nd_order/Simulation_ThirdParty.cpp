...
pair<int, int> payoff(int z, int x, int y, int P[], int D[], int N[], int benefit, int cost, double alpha, double eps, double ki, double tau) {
	...
	int observer = 0;
	do {
		observer = rand_int(0, z - 1);
	} while (observer == x || observer == y);

	int norm[4] = {};
	bitset<4>s3(N[observer]);
	for (int i = 0; i < 4; i++) {
		norm[i] = s3[i];
	}
		...
		for (int t = 0; t < g; t++) {
			...
			if (uni_rand() < mu) {
				...
				for (int i = 0; i < (2 * z); i++) {
					...
					int val;
					int ct;
					tie(val, ct) = payoff(z, a, c, P, D, N,benefit, cost, alpha, eps, ki, tau);
					fitness_a += val;
					if (ct != 3) {
						if (ct == 0) {
							coop += 2;
					...
					tie(val, ct) = payoff(z, b, c, P, D, N,benefit, cost, alpha, eps, ki, tau);
					fitness_b += val;
					if (ct != 3) {
						if (ct == 0) {
							coop += 2;
			...
			int index;
			int j;
			int G[64] = { 0 }; // # of G for each of 64 strat
			for (int i = 0; i < z; i++) {
				j = N[i];
				if (P[i] == 0) {
					index = 0 + j;
...
int main(int argc, char *argv[]) {
	...
	string name = "3p_runs_" + to_string(runs) + "_gen_" + to_string(g) + "_pop_" + to_string(z) + "_b_" + to_string(benefit) + "_c_" + to_string(cost) + "_eps_" + convert(eps) + "_alpha_" + convert(alpha) + "_ki_" + convert(ki) + "_tau_" + convert(tau) + "_mu_" + convert(mu) + "_seed_" + to_string(seed) + ".csv";
